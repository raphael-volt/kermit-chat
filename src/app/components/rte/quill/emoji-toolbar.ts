...
import { emojiMap, emojiList, Emoji } from "./emoji/list";
const fn_close = () => {
...
const fn_checkDialogOpen = (quill) => {
...
const fn_updateRange = (quill) => {
    let range = quill.getSelection();
    return range;
}

const fn_showEmojiPalatte = (quill) => {
    ...
    ele_emoji_area.style.position = 'absolute';

    let y = 10 + atSignBounds.top + atSignBounds.height
    const container = quill.container as HTMLElement
    let maxY = container.offsetHeight - 10
    const tah = 260
    let newY = y+tah
    if(newY > maxY) {
        y = maxY - tah
    }
    ele_emoji_area.style.top = y + "px";
...
const fn_emojiElementsToPanel = (type, panel, quill) => {
    ...
    let result: Emoji[] = fuse.search(type).map(result => result.item);
    result.sort((a: Emoji, b: Emoji) => {
    ...
    result.map(emoji => {
        ...
        if (customButton) {
            customButton.addEventListener('click', function () {
                ...
                quill.insertEmbed(range.index, 'emoji', emoji, "user")//Quill["sources"].USER);
...
const fn_updateEmojiContainer = (emojiFilter, panel, quill) => {
...
export class EmojiToolbar extends Module {
